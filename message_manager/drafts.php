...
<!-----PAGE LEVEL CSS END--->
                                        ...
                                        <h3 class="page-title">
                                                Entwürfe <small>Gespeicherte Nachrichten</small>
                                        </h3>
...
$sql = "select * from ".$db_suffix."draft_message where user_id = '".$_SESSION["user_id"]."' ORDER BY dm_created_time DESC LIMIT ".($start_show-1).", ".PER_PAGE_MSG;
$news_query = mysqli_query($db,$sql);
...
<thead>
                                ...
                                <tbody>
									...
									<tr <?php echo $active;?> data-href="<?php echo SITE_URL_ADMIN.'?mKey=sendmessage&dm_key='.$row->dm_key;?>">
										<td class="inbox-small-cells">
											<input value="<?php echo $row->dm_id;?>" type="checkbox" class="mail-checkbox" />
										</td>
										<td class="view-message hidden-xs message-show font-red">
											 [DRAFT]
										</td>
										...
										<td class="view-message text-left message-show">
											 <?php 
											  ...
											  if(date('d-m-Y')==date('d-m-Y', strtotime($row->dm_created_time)))
											  
												echo date('H:i', strtotime($row->dm_created_time));
												
											else
											
												echo date('d-m-Y H:i', strtotime($row->dm_created_time));	
				...
				$('#confirmation_all').on('show.bs.modal', function(e) {
					 ...
					 $('input:checkbox[class=mail-checkbox]:checked').each(function(){
						...
						id=id+$(this).val()+',';
					 	...
					 	$.ajax({
								   type: "POST",
								   url:  '<?php echo SITE_URL_ADMIN.'message_manager/'; ?>delete_message_drafts.php',
								   dataType: "text",
								   data: {id: id},
