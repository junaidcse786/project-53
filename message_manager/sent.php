...
<!-----PAGE LEVEL CSS END--->
                                        ...
                                        <h3 class="page-title">
                                                Gesendete Nachrichten<!-- <small>The list of messages already sent</small>-->
                                        </h3>
...
$sql = "select message_id from ".$db_suffix."message m
		Left Join ".$db_suffix."user u on m.message_receiver=u.user_id where m.message_sender = '".$_SESSION["user_id"]."' AND m.sender_delete='0' ORDER BY m.message_created_time DESC, m.message_seen ASC";
...
$sql = "select * from ".$db_suffix."message m
		Left Join ".$db_suffix."user u on m.message_receiver=u.user_id where m.message_sender = '".$_SESSION["user_id"]."' AND m.sender_delete='0' ORDER BY m.message_created_time DESC, m.message_seen ASC LIMIT ".($start_show-1).", ".PER_PAGE_MSG;
$news_query = mysqli_query($db,$sql);
...
<thead>
                                ...
                                <tbody>
									...
									<tr data-href="<?php echo SITE_URL_ADMIN.'?mKey=viewmessage&id='.$row->message_id;?>">
