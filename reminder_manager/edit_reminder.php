...
$sql = "select * from ".$db_suffix."reminder where r_id = $id AND user_id='".$_SESSION["user_id"]."' limit 1";				
$query = mysqli_query($db, $sql);

if(mysqli_num_rows($query) > 0)
{
	$content     = mysqli_fetch_object($query);
	$r_title       = $content->r_title;
	$r_desc    = $content->r_desc;
	$r_send_email    = $content->r_send_mail;
	$r_recipients    = $content->r_recipients;
	$parent=explode(',',$r_recipients);
	
	$r_remind_time_1 = date('Y-m-d', strtotime($content->r_remind_time));	
	$r_remind_time_2 = date('H:i:s', strtotime($content->r_remind_time));
}
else

	header('Location: '.SITE_URL_ADMIN.'?mKey=reminder&pKey=reminder');
...
{	
	...
	if($err == 0)
	{
		...
		$sql = "UPDATE ".$db_suffix."reminder SET r_title='$r_title', r_desc='$r_desc', r_remind_time='$r_remind_time', r_send_mail='$r_send_email', r_recipients='$r_recipients' WHERE r_id='$id'";
...
<!-----PAGE LEVEL CSS END--->
                                        ...
                                        <h3 class="page-title">
                                               Erinnerungsmail aktualisieren <small></small>
                                        </h3>
                                        <div class="page-bar">         
                                        <ul class="page-breadcrumb">
                                                ...
                                                <li>
                                                        <a href="<?php echo SITE_URL_ADMIN.'?mKey=reminder&pKey=reminder'; ?>"><?php echo $menus["reminder"]["reminder"]; ?></a>
														<i class="fa fa-angle-right"></i>
                                                </li>
												<li>
                                                        <a  href="#">Aktualsierung ID: <?php echo $id; ?></a>
                                                </li>
                                        </ul>
