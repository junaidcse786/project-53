...
$sql = "SELECT * FROM ".$db_suffix."voc_set ORDER BY voc_set_id DESC";
...
<!-----PAGE LEVEL CSS END--->
                                        ...
                                        <h3 class="page-title">
                                                <?php echo $menus["$mKey"]["$pKey"]; ?> <small>The list of vocabulary sets already created</small>
                                        </h3>
                               <div class="page-bar">         
            ...
            <div class="col-md-12">
               ...
               <div class="portlet box grey-cascade">
                  <div class="portlet-title">
                     <div class="caption"><i class="fa fa-table"></i>Vocab Sets</div>
                     <div class="actions">
                        <a href="<?php echo SITE_URL_ADMIN.'?mKey='.$mKey.'&pKey=addvocset'; ?>" class="btn blue"><i class="fa fa-plus"></i> Add new Set</a>
                        <div class="btn-group">
		   ...
		   ?>
                           ...
                           <tr class="odd gradeX">
                              <td><input type="checkbox" class="checkboxes" value="<?php echo $row->voc_set_id;?>" /></td>
                              <td><a href="<?php echo '?mKey='.$mKey.'&pKey=editvocset&id='.$row->voc_set_id;?>"><?php echo $row->voc_set_title; ?></a></td>
							  
                              <td><?php
							  $num_of_vocabs=mysqli_num_rows(mysqli_query($db,"SELECT voc_id FROM ".$db_suffix."voc WHERE voc_id IN (SELECT voc_id from ".$db_suffix."voc_relation where voc_set_id='$row->voc_set_id')"));
							  
							  ?>
                              
                              <a href="<?php echo '?mKey='.$mKey.'&pKey=voclist&id='.$row->voc_set_id.'&title='.$row->voc_set_title;?>" class="btn default btn-xs green-stripe">View Vocabularies </a> <span class="badge badge-danger"><?php echo $num_of_vocabs; ?></span></td>
                              
                              <td><?php echo $row->voc_set_level;?></td>
                              
                              <td><?php 
							  
							  		if($row->voc_set_status)
                              ...
                              <a href="<?php echo '?mKey='.$mKey.'&pKey=editvocset&id='.$row->voc_set_id;?>" class="btn default btn-xs purple"><i class="fa fa-edit"></i> Edit</a>
                              
                              <a data-href="<?php echo SITE_URL_ADMIN; ?>vocabulary_manager/delete_voc_set.php?id=<?php echo $row->voc_set_id;?>" data-toggle="modal" href="#" data-target="#confirmation" class="btn default btn-xs red delete"><i class="fa fa-trash"></i> Delete</a>
                              </td>  -->                             
                           </tr>
    ...
    <script>
				...
				$('#confirmation_all').on('show.bs.modal', function(e) {
					 ...
					 $('input:checkbox[class=checkboxes]:checked').each(function(){
						...
						$.ajax({
							   type: "POST",
							   url:  '<?php echo SITE_URL_ADMIN.'vocabulary_manager/delete_voc_set.php' ; ?>',
							   dataType: "text",
							   data: {id: id},
				...
				$('#confirmation_status').on('show.bs.modal', function(e) {
					 ...
					 var table_name='voc_set';
					 
					 var column_name='voc_set_status';
					 
					 var column_id='voc_set_id';
