...
$sql = "select * from ".$db_suffix."lang where lang_id = $id limit 1";				
$query = mysqli_query($db, $sql);

if(mysqli_num_rows($query) > 0)
{
	$content     = mysqli_fetch_object($query);
	$title       = $content->lang_title;
	$old_title   = $title;
	$published   = $content->lang_status;	
} 
...
{	
	...
	if($err == 0)
	{
		...
		$sql = "UPDATE ".$db_suffix."lang SET lang_title='$title', lang_status='$published' where lang_id = $id";
		...
		{		
			...
			$sql = "RENAME TABLE ".$db_suffix.strtolower($old_title)." to ".$db_suffix.strtolower($title);
			mysqli_query($db,$sql);	   
		}else{
			$alert_box_show="show";
			$alert_type="danger";
...
<!-----PAGE LEVEL CSS END--->
                                        ...
                                        <h3 class="page-title">
                                                Edit Vocabulary Language <small>Here New Languages for vocabulary can be updated</small>
                                        </h3>
                                        <div class="page-bar">         
                                        <ul class="page-breadcrumb">
                                                ...
                                                <li>
                                                        <a href="<?php echo SITE_URL_ADMIN.'?mKey=general&pKey=language'; ?>">Language List</a>
														<i class="fa fa-angle-right"></i>
                                                </li>
												<li>
